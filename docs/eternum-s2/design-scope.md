...
The **bridge** is the gateway between in-game balances and a player's external wallet. A **World Bank** is Eternum's
in-game trading venue. These are separate systems: the Bank swaps one asset for another inside the economy, while the
...
The Bank is an automated market: its two reserves price a swap using the constant-product rule `x*y=k`. The Bank
